...
def create_model(x, dimensions):
    ...
    hidden_one = tf.add(tf.matmul(x, weights['hidden_one']), biases['hidden_one'])
    hidden_one = tf.nn.softmax(hidden_one)

    hidden_two = tf.add(tf.matmul(hidden_one, weights['hidden_two']), biases['hidden_two'])
    hidden_two = tf.nn.softmax(hidden_two)

    output = tf.add(tf.matmul(hidden_two, weights['output']), biases['output'])
...
def create_weights(dimensions, method):
    return {
        'hidden_one': create_matrix_distribution(dimensions['num_input'], dimensions['num_hidden_one'], method),
        'hidden_two': create_matrix_distribution(dimensions['num_hidden_one'], dimensions['num_hidden_two'], method),
        'output': create_matrix_distribution(dimensions['num_hidden_two'], dimensions['num_output'], method)
    }

def create_biases(dimensions, method):
    return {
        'hidden_one': create_matrix_distribution(1, dimensions['num_hidden_one'], method),
        'hidden_two': create_matrix_distribution(1, dimensions['num_hidden_two'], method),
...
def main(argv=None):
    ...
    num_hidden_one = FLAGS.num_hidden_one
    num_hidden_two = FLAGS.num_hidden_two
    ...
    dimensions = {
        ...
        'num_hidden_one': num_hidden_one,
        'num_hidden_two': num_hidden_two,
